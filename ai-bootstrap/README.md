...
>
> 当前版本：1.13.1 ｜ 执行规范：[SKILL.md](./SKILL.md)

## 版本记录

- `1.13.1`（当前）：环境基线落盘 + UI 字体默认跳过：生成器按 Blueprint 技术栈推导「环境与工具链基线」（Spring Boot 系锁定 **JDK 17（最稳）+ Maven 3.9+（必须支持）**，`mvn -f backend/pom.xml` 构建；前端 Node.js 20+ LTS + pnpm），写入 AGENTS.md / PROJECT_PROFILE.md / DESIGN.md / README.md，AI 安装环境时直接参考，不再靠对话口口相传；react-springboot / vue-springboot 两个 Java Blueprint 声明 `environment` 元数据（`backend_runtime: JDK 17 + Maven 3.9+`）；**UI 字体默认跳过**——vfonts（Lato/Inter）与 @fontsource-variable/inter 均为 Google 字体打包，国内下载慢/易失败，初始化不再强制安装（依赖与 import 以注释占位，校验不视为缺失），需要时 `--fonts` 显式开启（`AI_BOOTSTRAP_FONTS=1` 等效）。
- `1.13.0`：Java/Maven 后端补齐 + vue-springboot Blueprint：react-springboot 新增 Spring Boot 后端 starter（Spring Initializr 官方范式、JDK 17、H2 默认开箱即跑、Dockerfile、`/api/health` + `/api/jobs` thin API），此前该 Blueprint 只生成前端；新增 `vue-springboot`（Vue3 + Element Plus + Spring Boot）覆盖国内企业级主流组合；`vue-element-plus-web` 的 tsconfig 改为自包含（不再依赖 workspace 根 tsconfig.base.json，任意挂载深度可构建）；`build_smoke.py` 新增 Maven 目标支持（自动识别 pom.xml，`mvn -q -DskipTests package`）。
- `1.12.0`：官方 DEMO 对齐清单 + 构建期冒烟：`framework_gate.py` 为 6 个 UI 栈注册 `official_demo_url` / `official_docs_url`（Nuxt UI / Vant / Element Plus / Ant Design / shadcn-ui / Naive UI），生成项目的 README.md 与 `docs/00-research/design-token-spec.md` 自动附「与官方 DEMO 对齐」验收清单（官方 demo/starter 链接、官方安装范式、主题令牌入口、starter 目录、`ui-stack-conformance` 门禁）；新增 `scripts/build_smoke.py` 对生成项目做真实 `pnpm install` + 构建冒烟（workspace 根只 install、各 app 单独构建；`--plan` 只打印计划、`--skip-install` 跳过安装、`--only` 白名单、`--json` 输出），与 `smoke.py` 的运行时冒烟互补，验证 starter「生成即能跑」（真实 build 已覆盖 nuxt-ai-fullstack / next-fullstack / react-fastapi / vue-element-plus-nitro / vue-django / uni-app-nitro，uni-app starter 依官方要求补 H5 `index.html` 入口、pinia 固定 2.3.1、workspace 根补 `.npmrc`（`shamefully-hoist=true` + `auto-install-peers=true`）修复 `@vant/use` 等子依赖解析）。
- `1.11.0`：shadcn/ui 补全 Next.js App Router 官方安装范式 starter：`next-fullstack` Blueprint 内置 `next-shadcn-web` starter（`apps/web/`，Next.js 15 + React 19 + Tailwind v4 `@tailwindcss/postcss` + `app/` App Router 布局（`app/globals.css` 主题 CSS 变量 + `app/layout.tsx` 元数据 + 路由组 `(app)` 应用外壳）+ `components/` + `lib/utils.ts` + `components.json`（`rsc: true`）+ `components/ui/` 源码拷贝组件 + `'use client'` 交互组件 + `@/*` 路径别名，登录 → 工作台 → 岗位 CRUD 开箱即跑）；`ui-stack-conformance` 的 shadcn 检查升级支持 Next.js 官方布局（`app/globals.css` / `components/ui` / 多根语义类名扫描），并按项目布局自动标注 `details.starter`（`next-shadcn-web` / `react-shadcn-web`）。
- `1.10.0`：shadcn/ui 与 Naive UI 落地「生成即能跑 + 校验即通过」的瘦 DEMO starter：`react-fastapi` Blueprint 内置 `react-shadcn-web` starter（`frontend/`，Vite + React 19 + Tailwind v4 + `@tailwindcss/vite` + `components.json` + `src/index.css` 主题 CSS 变量 + `src/components/ui/` 源码拷贝组件（button/card/input/badge/table/dialog）+ `src/lib/utils.ts` `cn()` + `@/*` 别名 + 语义类名页面，登录 → 工作台 → 岗位 CRUD 开箱即跑）；`vue-django` Blueprint 内置 `vue-naive-web` starter（`frontend/`，Vue3 + Vite + Naive UI 2.x：零 CSS 导入 + `n-config-provider :theme-overrides` + `GlobalThemeOverrides` 集中 `theme.ts` + `zhCN`/`dateZhCN` locale + `n-*` 组件基线页面）；`ui-stack-conformance` 对 shadcn/naive 的官方范式检查升级为「生成即通过」（starter 正样零 warning），并保留反模式拦截（`babel-plugin-import`、`naive-ui/dist` 全量 CSS）。
